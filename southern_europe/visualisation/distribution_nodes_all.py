...
if 'Annual_Flu' in nodes.columns:
    ...
    for idx, row in nodes.iterrows():
        ...
        # Use black edge color for all nodes (no red outlines)
        edge_color = 'black'
        edge_width = 1.5
    ...
    # Note: Removed the "Selected Node" legend item since we're not showing red outlines
    ...
    for node_type, color in node_colors.items():
        ...
        if len(type_nodes) > 0:
            ...
            # Plot all nodes with black edge color (no distinction for selected nodes)
            if node_type == 'Storage':
                type_nodes.plot(ax=ax, color=color, markersize=100, alpha=0.8,
                                edgecolor='black', marker='s')
            elif node_type == 'Transport':
                type_nodes.plot(ax=ax, facecolor=color, markersize=100, alpha=0.8,
...
plt.savefig('northern_italy_network_no_red_outlines.png', dpi=600, bbox_inches='tight')
plt.show()