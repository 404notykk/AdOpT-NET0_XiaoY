...
greece = gpd.read_file(path_files_gis / "greece_WGS1984.shp") # greece boundary
fishnet = gpd.read_file(path_files_gis/"fishnet_greece_25km.shp").reset_index().rename(
    columns={"index": "GRID_OID"})
soil_data = pd.read_csv(path_files_grids / "soil_type_grids_greece.csv")
anthro_data = pd.read_csv(path_files_grids / "anthropisation_grids_greece.csv")
morpho_data = pd.read_csv(path_files_grids / "morphological_feature_grids_greece.csv")
...
fishnet_clipped = gpd.clip(fishnet, greece)
...
plt.savefig('greece_incremental_cost_factors_with_integrated.png', dpi=600, bbox_inches='tight')
plt.show()

# ——— Plot 2: single map for the total cost factor (kept for comparison/standalone use) ———
fig, ax = plt.subplots(1, 1, figsize=(8, 8))
greece.boundary.plot(ax=ax, color='black', linewidth=1)
...
plt.savefig('greece_cost_factor.png', dpi=600, bbox_inches='tight')
plt.show()
